...
<div>
	<div id="panels" class="flex flex-col h-screen">
		...
		<div class="panel-1 tab-content" v-show="!tabClicked">
			<div class="mx-auto text-center text-xs sm:text-sm w-full lg:w-11/12">
				<div class="flex flex-row justify-end mb-10 md:mb-0">
					...
					<order-card v-for="order in this.orders" v-on:reset="reset" :key="order.id" :order="order" />
			</div>
...
export default {
	...
	methods: {
		...
		infiniteHandler($state) {
			this.$store.dispatch('order/fetchOrders', this.page)
			.then((response) => {
